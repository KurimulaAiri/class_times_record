...
<template>
    <view class="component-container">
        ...
        <view class="section select-student" @tap="navigateToSelect">
            ...
            <view class="content">
                <text v-if="student.studentName" class="name">{{ student.studentName }}</text>
        ...
        <view class="list-header" v-if="classes && classes.length > 0">
            ...
            <view class="select-all-btn" @tap="toggleSelectAll">
                ...
                ></uni-icons>
                <text class="select-all-text" :class="{ active: isAllSelected }">全选</text>
            </view>
        </view>
            ...
            >
                <view class="course-info">
                    ...
                    <view class="course-detail">
                        <text>任教老师：{{ item.teachers.map((t) => t.username).join("、") }}</text>
                        <text>课程名称：{{ item.courseName }}</text>
                        <text>该种课程剩余课时：{{ item.courseRecord?.courseRestTime ?? 0 }}</text>
                    </view>
                </view>
...
<script setup lang="ts">
    ...
    const isAllSelected = computed(() => {
        ...
        return classes.value.every(item => selectedMap.value[item.id] !== undefined);
    });
    ...
    // 页面通信监听
    uni.$on("updateStudent", async (data: any) => {
        student.value = data;
        selectedMap.value = {};
        await loadData();
    });
