        ...
        >
            ...
            <template #group-1>
                ...
                <view class="add-placeholder" @tap="addSchedule">
                    ...
                    <text>{{ form.schedules.length > 0 ? "追加一组上课时段" : "添加上课日程时段" }}</text>
                </view>
            </template>
        </FormPage>
...
<script setup lang="ts">
    ...
    // 2. 【核心破局点】：彻底干掉 computed！将 groups 改为固定内存指针的普通配置项
    ...
    onLoad(() => {
        ...
        uni.$on("updateCourse", (res: CourseResponse) => {
            console.log("接收到跨页选择的课程:", res);
            form.value.courseId = res.id;
            
            // 【精准拦截】：不重算数组，只改对应 Item 的文本属性，打破组件刷新陷阱
            if (groups[0]?.items?.[1]) {
                groups[0].items[1].pickerText = res.courseName;
            }
        ...
        uni.$on("updateTeachers", (res: TeacherResponse[]) => {
            console.log("接收到跨页选择的老师:", res);
            
            // 规范化后端传输所需属性
            form.value.teachers = res.map(t => ({
